...
@pytest.mark.asyncio
async def test_ground_query_parses_intent(patch_server):
...
@pytest.mark.asyncio
async def test_ground_query_fails_open(patch_server):
    async def fake(instruction, max_tokens=400):
...
@pytest.mark.asyncio
async def test_ground_query_caches(patch_server):
    calls = {"n": 0}
    async def fake(instruction, max_tokens=400):
        calls["n"] += 1
...
@pytest.mark.asyncio
async def test_gate_drops_rejected_images(patch_server):
    ...
    patch_server("_fetch_image_data_url", fake_fetch)
    patch_server("_fast_multimodal_json", fake_vision)
...
@pytest.mark.asyncio
async def test_gate_fails_open_when_no_image_fetches(patch_server):
    async def fake_fetch(client, url, max_bytes=1_800_000):
...
@pytest.mark.asyncio
async def test_gate_fails_open_on_vision_outage(patch_server):
    async def fake_fetch(client, url, max_bytes=1_800_000):
...
@pytest.mark.asyncio
async def test_gate_min_keep_prevents_empty_grid(patch_server):
    async def fake_fetch(client, url, max_bytes=1_800_000):
        return "data:image/jpeg;base64,AAAA"
    async def fake_vision(content, max_tokens=300, temperature=0.1):
...
@pytest.mark.asyncio
async def test_consistency_skips_single_widget(patch_server):
    async def fake(*a, **k):
...
@pytest.mark.asyncio
async def test_consistency_drops_offsubject_widget(patch_server):
...
@pytest.mark.asyncio
async def test_consistency_fails_open_when_all_dropped(patch_server):
    async def fake(instruction, max_tokens=120):
...
@pytest.mark.asyncio
async def test_consistency_fails_open_on_model_error(patch_server):
    async def fake(instruction, max_tokens=120):
        return None
    patch_server("fast_llm_json", fake)
    good = [("a", "a", {"title": "x"}, None), ("b", "b", {"title": "y"}, None)]
...
@pytest.mark.asyncio
async def test_build_image_config_searches_expanded_query_and_gates(patch_server):
    seen = {}
    ...
    patch_server("web_search", fake_search)
    patch_server("_enrich_news", fake_enrich)
    patch_server("filter_images_by_relevance", fake_gate)
